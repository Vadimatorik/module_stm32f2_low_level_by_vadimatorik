...
struct __attribute__( ( packed ) ) port_registers_struct {
    uint32_t   mode;       // Регистр выбора режима работы выводов.
    uint32_t   otype;      // Регистр выбора режима выхода
                           // ( в случае, если вывод настроен как выход ).
    uint32_t   ospeede;    // Регистр выбора скорости выводов.
    uint32_t   pupd;       // Регистр включения подтяжки выводов.
    uint32_t   id;         // Регистр с текущими данными на входе вывода.
    uint32_t   od;         // Регистр с выставленными пользователем на выход данными
                           // ( в случае, если вывод настроен как выход ).
    uint32_t   bsr;        // Регистр быстрой установки состояния выводов
                           // ( когда вывод настроен как выход ).
    uint32_t   lck;        // Регистр блокировки настроек.
    uint32_t   afl;        // Младший регистр настройки альтернативных функций выводов.
    uint32_t   afh;        // Старший регистр настройки альтернативных функций выводов.
};
